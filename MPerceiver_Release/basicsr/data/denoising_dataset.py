...
class denoising_dataset_train(data.Dataset):
    ...
    def __init__(self, opt):
        super(denoising_dataset_train, self).__init__()
        self.opt = opt
        ...
        if 'gt_path' in opt:
            if isinstance(opt['gt_path'], str):
                for img_type in opt['image_type']:
                    self.paths_gt.extend(sorted([str(x) for x in Path(opt['gt_path']).glob('*.'+img_type)]))
            else:
                for img_type in opt['image_type']:
                    self.paths_gt.extend(sorted([str(x) for x in Path(opt['gt_path'][0]).glob('*.'+img_type)]))
                    if len(opt['gt_path']) > 1:
                        for i in range(len(opt['gt_path'])-1):
                            self.paths_gt.extend(sorted([str(x) for x in Path(opt['gt_path'][i+1]).glob('*.'+img_type)]))
    ...
    def __getitem__(self, index):
        ...
        # img_lq = img_gt + np.random.normal(0, sigma, img_gt.shape)
        # img_lq = np.clip(img_lq, 0, 1)
        img_gt = np.frombuffer(img_bytes_gt, np.uint8)
        img_gt = cv2.imdecode(img_gt, cv2.IMREAD_COLOR)
        sigma = np.random.choice([15,25,50])
        img_lq = img_gt + np.random.normal(0, sigma, img_gt.shape)
...
class denoising_dataset_test(data.Dataset):
    """Modified dataset based on the dataset used for Real-ESRGAN model:
    Real-ESRGAN: Training Real-World Blind Super-Resolution with Pure Synthetic Data.
    ...
    def __init__(self, opt):
        super(denoising_dataset_test, self).__init__()
        self.opt = opt
        self.file_client = None
        self.io_backend_opt = opt['io_backend']
        if 'crop_size' in opt:
        ...
        if 'lq_path' in opt:
            if isinstance(opt['lq_path'], str):
                for img_type in opt['image_type']:
                    self.paths_lq.extend(sorted([str(x) for x in Path(opt['lq_path']).glob('*.'+img_type)]))
            else:
                for img_type in opt['image_type']:
                    self.paths_lq.extend(sorted([str(x) for x in Path(opt['lq_path'][0]).glob('*.'+img_type)]))
                    if len(opt['lq_path']) > 1:
                        for i in range(len(opt['lq_path'])-1):
                            self.paths_lq.extend(sorted([str(x) for x in Path(opt['lq_path'][i+1]).glob('*.'+img_type)]))
