...
import open_clip
class CLIP_Predictor(nn.Module):
    ...
    def forward(self,image):
        fea = self.model(image) # b,1024
        cls = self.fc2(self.relu(self.fc1(fea)))
        return fea, cls

from ldm.modules.attention import MemoryEfficientCrossAttention
class CM_Inv(torch.nn.Module):
    """Projection Model"""
    def __init__(self, dim=1024, clip_extra_context_tokens=16, num_class=9):
        super().__init__()
        self.dim = dim
        self.clip_extra_context_tokens = clip_extra_context_tokens
        self.proj = torch.nn.Linear(dim, self.clip_extra_context_tokens * dim)
        self.norm = torch.nn.LayerNorm(dim)
        self.attns = nn.ModuleList([MemoryEfficientCrossAttention(dim,None,8,int(dim//8)) for _ in range(num_class)])
        
    def forward(self, image_embeds):
        embeds = image_embeds #b 1024
        clip_extra_context_tokens = self.proj(embeds).reshape(-1, self.clip_extra_context_tokens, self.dim)#b n 1024
        clip_extra_context_tokens = self.norm(clip_extra_context_tokens)
        results = []
        for attn in self.attns:
            results.append(attn(clip_extra_context_tokens))
        return results

class MPerceiver(DDPM):
    """main class"""
    def __init__(self,
                 first_stage_config,
                 cond_stage_config,
        ...
        # self.caption_model = Blip2ForConditionalGeneration.from_pretrained("Salesforce/blip2-opt-2.7b", torch_dtype=torch.float16)

        self.classifier = CLIP_Predictor(9)
        self.classifier.load_state_dict(torch.load('/data2/yuang.ai/code/MPerceiver/ckpt/9class_classifier.pth',map_location=torch.device('cpu')))
        self.classifier.freeze()

        self.CM_Inv = CM_Inv(1024,16,9)
        self.CM_Inv.train()
        for param in self.CM_Inv.parameters():
            param.requires_grad = True
        
        #textual prompt
        tp_vectors = torch.empty((9, 16, 1024), dtype=torch.float32)
        nn.init.normal_(tp_vectors, std=0.02)
        self.tp = nn.Parameter(tp_vectors)
        ...
        # for name, param in self.caption_model.named_parameters():
        #     param.requires_grad = False
    ...
    def get_input(self, batch, k=None, return_first_stage_outputs=False, force_c_encode=False,
        ...
        im_gt = batch['gt']
        # if not isinstance(im_gt, list):
        #     im_gt = [im_gt]
        # im_gt = torch.cat(im_gt, dim=0)
        # im_gt = im_gt.cuda()
        # im_gt = im_gt.to(memory_format=torch.contiguous_format).float()
        if len(im_gt.shape) == 5:
            im_gt = im_gt.flatten(0, 1)

        im_lq = batch['lq']
        if len(im_lq.shape) == 5:
            im_lq = im_lq.flatten(0, 1)
        # if not isinstance(im_lq, list):
        #     im_lq = [im_lq]
        # im_lq = torch.cat(im_lq, dim=0)
        # im_lq = im_lq.cuda()
        # im_lq = im_lq.to(memory_format=torch.contiguous_format).float()
        im_lq_clip = batch['lq_clip']
        if len(im_lq_clip.shape) == 5:
            im_lq_clip = im_lq_clip.flatten(0, 1)
        ...
        if bs is not None:
            x = x[:bs]
            y = y[:bs]
            im_lq_clip = im_lq_clip[:bs]
        x = x.to(self.device)
        y = y.to(self.device)
        im_lq_clip = im_lq_clip.to(self.device)

        img_fea, de_probs = self.classifier(im_lq_clip)
        img_fea = img_fea.detach()
        de_probs = de_probs.detach()
        img_fea = self.CM_Inv(img_fea)
        # sft = nn.Softmax(dim=1)
        # de_probs = sft(de_probs)
        ...
        # if True:
        #     with torch.no_grad():
        #         gt_caption = batch['gt_caption'].cuda()
        #         gt_caption = self.caption_processor(gt_caption, return_tensors="pt").to('cuda', torch.float16)
        #         gt_caption = self.caption_model.generate(**gt_caption, max_new_tokens=30)
        #         text_cond = self.caption_processor.batch_decode(gt_caption, skip_special_tokens=True)
        #     # text_cond = ['without snow, ' + x.strip() for x in text_cond]   
        #     text_cond = [','+x.strip() + '.' for x in text_cond]
    ...
    def shared_step(self, batch, **kwargs):
        #enc_lq, text, enc_gt, prob, img_fea
        x, c, gt, probs, img_fea = self.get_input(batch, self.first_stage_key)
        loss = self(x, c, gt, probs, img_fea)
        return loss

    def get_condtion_prompt(self,c,probs,img_fea):
        sft = nn.Softmax(dim=1)
        probs = sft(probs)
        c_list = []
        for i in range(probs.size(1)):
            c_list.append(self.cond_stage_model(c,self.tp[i],img_fea[i]).unsqueeze(1))
        c_all = torch.cat(c_list,dim=1)
        c_all = probs[:,:,None,None]*c_all
        c_all = torch.sum(c_all,dim=1)
        return c_all

    def forward(self, x, c, gt,probs,img_fea, *args, **kwargs):
        #enc_lq, text, enc_gt, lq_image
        index = np.random.randint(0, self.num_timesteps, size=x.size(0))
        t = torch.from_numpy(index)
        t = t.to(self.device).long()
        ...
        if self.model.conditioning_key is not None:
            ...
            if self.cond_stage_trainable:
                ...
                # c = self.cond_stage_model(c)
                c = self.get_condtion_prompt(c,probs,img_fea)
            if self.shorten_cond_schedule:  # TODO: drop this option
                print(s)
                tc = self.cond_ids[t].to(self.device)
                c = self.q_sample(x_start=c, t=tc, noise=torch.randn_like(c.float()))
        if self.test_gt:
            ...
            struc_c = self.structcond_stage_model(x, t_ori,probs)
            #enc_gt, text, lq_con,lq_image, t, t_ori, enc_lq
        return self.p_losses(gt, c, struc_c, t, t_ori, x, *args, **kwargs)
    ...
    @torch.no_grad()
    def p_sample_loop(self, cond, struct_cond,probs, shape, return_intermediates=False,
                      x_T=None, verbose=True, callback=None, timesteps=None, quantize_denoised=False,
                      mask=None, x0=None, img_callback=None, start_T=None,
                      log_every_t=None, time_replace=None, adain_fea=None, interfea_path=None):
        ...
        for i in iterator:
            ...
            if t_replace is not None:
                if start_T is not None:
                    if self.ori_timesteps[i] > start_T:
                         continue
                struct_cond_input = self.structcond_stage_model(struct_cond, t_replace,probs)
            else:
                if start_T is not None:
                    if i > start_T:
                        continue
                struct_cond_input = self.structcond_stage_model(struct_cond, ts,probs)
    ...
    @torch.no_grad()
    def sample(self, cond, struct_cond,probs, batch_size=16, return_intermediates=False, x_T=None,
               verbose=True, timesteps=None, quantize_denoised=False,
               mask=None, x0=None, shape=None, time_replace=None, adain_fea=None, interfea_path=None, start_T=None, **kwargs):
    ...
    @torch.no_grad()
    def log_images(self, batch, N=11, n_row=4, sample=True, ddim_steps=200, ddim_eta=1., return_keys=None,
                   quantize_denoised=True, inpaint=False, plot_denoise_rows=False, plot_progressive_rows=False,
                   plot_diffusion_rows=False, **kwargs):
        ...
        log = dict()
        z, c_lq, z_gt, probs,img_fea, x, gt, yrec, xc = self.get_input(batch, self.first_stage_key,
                                           return_first_stage_outputs=True,
                                           force_c_encode=True,
                                           return_original_cond=True,
                                           bs=N, val=True)
        ...
        # c = self.cond_stage_model(c_lq)
        c = self.get_condtion_prompt(c_lq, probs,img_fea)
        ...
        if sample:
            ...
            with self.ema_scope("Plotting"):
                ...
                samples, z_denoise_row = self.sample(cond=c, struct_cond=struct_cond,probs=probs, batch_size=N, timesteps=cur_time_step, return_intermediates=True, time_replace=self.time_replace)
            x_samples = self.decode_first_stage(samples)
            log["samples"] = x_samples
            if plot_denoise_rows:
                denoise_grid = self._get_denoise_row_from_list(z_denoise_row)
    ...
    def configure_optimizers(self):
        ...
        params = params + list(self.cond_stage_model.parameters())
        params = params + list(self.structcond_stage_model.parameters())
        params = params + [self.tp]
        params = params + list(self.CM_Inv.parameters())
        if self.learn_logvar:
            assert not self.learn_logvar
            print('Diffusion model optimizing logvar')
            params.append(self.logvar)
