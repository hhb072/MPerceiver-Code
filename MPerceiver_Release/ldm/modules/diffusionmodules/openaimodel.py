...
from ldm.modules.attention import SpatialTransformer, SpatialTransformerV2, SpatialTransformerV2_win
...
class ResBlockDual(TimestepBlockDual):
    ...
    def __init__(
        ...
        self.spade = SPADE(self.out_channels, context_dim)
        # if self.out_channels ==320:
        #     ckpt=True
        # else:
        #     ckpt=False
        # self.cross_attn = SpatialTransformerV2_win(in_channels=self.out_channels,n_heads=self.out_channels//64,d_head=64,depth=1,context_dim=context_dim,disable_self_attn=False,use_linear=True,use_checkpoint=ckpt,win=win_attn)
        self.out_layers = nn.Sequential(
            normalization(self.out_channels),
            nn.SiLU(),
            nn.Dropout(p=dropout),
    ...
    def _forward(self, x, emb, s_cond):
        ...
        if self.use_scale_shift_norm:
            ...
            h = h + emb_out
            h = self.out_layers(h)
        # h = self.cross_attn(h, s_cond)
        h = self.spade(h,s_cond)
        return self.skip_connection(x) + h


class ResBlockDual_Cross(TimestepBlockDual):
    """
    A residual block that can optionally change the number of channels.
    :param channels: the number of input channels.
    :param emb_channels: the number of timestep embedding channels.
    ...
    def __init__(
        ...
        # Here we use the built component of SPADE, rather than SFT. Should have no significant influence on the performance.
        self.spade = SPADE(self.out_channels, 4)
        # if self.out_channels ==320:
        #     ckpt=True
        # else:
        #     ckpt=False
        # self.cross_attn = SpatialTransformerV2(in_channels=self.out_channels,n_heads=self.out_channels//64,d_head=64,depth=1,context_dim=4,disable_self_attn=False,use_linear=True,use_checkpoint=False)
        self.out_layers = nn.Sequential(
            normalization(self.out_channels),
            nn.SiLU(),
            nn.Dropout(p=dropout),
    ...
    def _forward(self, x, emb, s_cond):
        ...
        if self.use_scale_shift_norm:
            ...
            h = self.out_layers(h)
        # h = self.cross_attn(h, s_cond)
        # s_cond = rearrange(s_cond,'b c h w -> b (h w) c')
        mp = nn.MaxPool2d(kernel_size=2)
        s_cond = {'64':s_cond,'32':mp(s_cond),'16':mp(mp(s_cond)),'8':mp(mp(mp(s_cond)))}
        h = self.spade(h,s_cond)
        return self.skip_connection(x) + h
...
class AttentionBlock_wo_res(nn.Module):
    """
    An attention block that allows spatial positions to attend to each other.
    Originally ported from here, but adapted to the N-d case.
    https://github.com/hojonathanho/diffusion/blob/1e0dceb3b3495bbe19116a5e1b3596cd0706c543/diffusion_tf/models/unet.py#L66.
...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps=None, context=None, y=None,**kwargs):
        ...
        if self.predict_codebook_ids:
            ...
            return self.out(h)
def Normalize(in_channels, num_groups=49):
...
class UNetModelDualcondV2(nn.Module):
    ...
    def forward(self, x, timesteps=None, context=None, struct_cond=None, y=None,**kwargs):
        ...
        # b,c,H,W = lq.shape
        # lq = lq.view(b,c*64,H//8,W//8)
        # enc_feat = self.encode_enc_1(torch.cat([h,lq], dim=1))
        # enc_feat = self.encode_enc_2(enc_feat)
        # enc_feat = self.encode_enc_3(enc_feat)
        # h = h + self.layerscale*enc_feat

        for i, module in enumerate(self.input_blocks):
            h = module(h, emb, context, struct_cond)
            # if i == 0:
            #     h = h + self.cross_attn(h,lq)
            hs.append(h)
...
class IRAdapter(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
            ...
            for _ in range(num_res_blocks):
                ...
                if ds in attention_resolutions:
                    layers.append(
                        ...
                        # SwinTransformerBlock(dim=ch,input_resolution=(512//int(2**level),512//int(2**level)),num_heads=4,window_size=8,shift_size=4 if level ==1 else 0,mlp_ratio=2)
                    )
                self.input_blocks.append(TimestepEmbedSequential(*layers))
                self._feature_size += ch
            if level != len(channel_mult) - 1:
        ...
        self.middle_block = TimestepEmbedSequential(
            ...
            # SwinTransformerBlock(dim=ch,input_resolution=(512//int(2**level),512//int(2**level)),num_heads=4,window_size=8,shift_size=4,mlp_ratio=2),
            ResBlock(
                ch,
                time_embed_dim,
                dropout,
        ...
        visual_prompt = torch.empty((len(input_block_chans),9, 1, 24,512)) #4, class, b, n, c
        nn.init.normal_(visual_prompt, std=0.02)
        self.visual_prompt = nn.Parameter(visual_prompt)
        self.prompt_attns = nn.ModuleList([])
        for i in range(len(input_block_chans)):
            self.prompt_attns.append(
                MemoryEfficientCrossAttention(query_dim=input_block_chans[i],context_dim=512,heads=8,dim_head=64)
            )
        self.norm1 = nn.ModuleList([])
        for i in range(len(input_block_chans)):
            self.norm1.append(nn.LayerNorm(input_block_chans[i]))
        self.norm2 = nn.ModuleList([])
        for i in range(len(input_block_chans)):
            self.norm2.append(nn.LayerNorm(512))
    ...
    def forward(self, x, timesteps,probs):
        """
        Apply the model to an input batch.
        :param x: an [N x C x ...] Tensor of inputs.
        :param timesteps: a 1-D batch of timesteps.
        ...
        for i in range(len(result_list)):
            b,c,h,w = result_list[i].shape
            result_list[i] = result_list[i].reshape(b,c,h*w).permute(0,2,1) # b,n,c
            res = result_list[i]
            for j in range(num_class):
                result_list[i] = result_list[i] + probs[:,j:j+1,None] * self.prompt_attns[i](self.norm1[i](res),self.norm2[i](self.visual_prompt[i][j].repeat(b,1,1)))
            result_list[i] = result_list[i].permute(0,2,1).reshape(b,c,h,w)
...
class EncoderUNetModelWT(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.model_lq = EncoderUNetModel_lq(image_size,
        3,
        model_channels,
        out_channels,
        num_res_blocks,
        ...
        self.model_lqf = EncoderUNetModel_lqf(image_size,
        in_channels,
        model_channels,
        out_channels,
        num_res_blocks,
    ...
    def convert_to_fp16(self):
        ...
        Convert the torso of the model to float16.
        """
        self.model_lq.apply(convert_module_to_f16)
        self.model_lqf.apply(convert_module_to_f16)

    def convert_to_fp32(self):
        """
        Convert the torso of the model to float32.
        """
        self.model_lq.apply(convert_module_to_f32)
        self.model_lqf.apply(convert_module_to_f32)

    def forward(self, lq,lqf, timesteps):
        """
        Apply the model to an input batch.
        :param x: an [N x C x ...] Tensor of inputs.
        :param timesteps: a 1-D batch of timesteps.
        :return: an [N x K] Tensor of outputs.
        """
        results_lq = self.model_lq(lq,timesteps)
        results_lqf = self.model_lqf(lqf,timesteps)
        results = {}
        for key in results_lq.keys():
            results[key] =results_lq[key]
        for key in results_lqf.keys():
            results[key] =results_lqf[key]
        return results
