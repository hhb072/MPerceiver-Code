...
def cal_color_loss(x, y):
    x = rearrange(x, 'b c h w -> (b h w) c')
    y = rearrange(y, 'b c h w -> (b h w) c')
    return nn.CosineEmbeddingLoss()(x, y, torch.ones(x.shape[0], device=x.device))

class LPIPSWithDiscriminator(nn.Module):
    def __init__(self, disc_start, logvar_init=0.0, kl_weight=1.0, pixelloss_weight=1.0,
                 disc_num_layers=3, disc_in_channels=3, disc_factor=1.0, disc_weight=1.0,
                 perceptual_weight=0.1, use_actnorm=False, disc_conditional=False,
                 disc_loss="hinge"):
    ...
    def forward(self, inputs, reconstructions, posteriors, optimizer_idx,
        ...
        color_loss = cal_color_loss(inputs.contiguous(), reconstructions.contiguous())
        ...
        if optimizer_idx == 0:
            ...
            if self.kl_weight>0:
                loss = weighted_nll_loss + self.kl_weight * kl_loss + d_weight * disc_factor * g_loss + self.color_loss_weight * color_loss
                log = {"{}/total_loss".format(split): loss.clone().detach().mean(), "{}/logvar".format(split): self.logvar.detach(),
                       "{}/kl_loss".format(split): kl_loss.detach().mean(), "{}/nll_loss".format(split): nll_loss.detach().mean(),
                       "{}/rec_loss".format(split): rec_loss.detach().mean(),
                       "{}/color_loss".format(split): color_loss.detach().mean(),
                ...
                if return_dic:
                    ...
                    loss_dic['color_loss'] = color_loss.detach().mean()
                    loss_dic['d_weight'] = d_weight.detach()
                    loss_dic['disc_factor'] = torch.tensor(disc_factor)
                    loss_dic['g_loss'] = g_loss.detach().mean()
            else:
                loss = weighted_nll_loss + d_weight * disc_factor * g_loss + self.color_loss_weight * color_loss
                log = {"{}/total_loss".format(split): loss.clone().detach().mean(), "{}/logvar".format(split): self.logvar.detach(),
                ...
                if return_dic:
                    ...
                    loss_dic['color_loss'.format(split)] = color_loss.detach().mean()
