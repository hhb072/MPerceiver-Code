...
class FrozenOpenCLIPEmbedder(AbstractEncoder):
    ...
    def forward(self, text, text_prompt=None, image_prompt=None):
        if text_prompt == None:
            tokens = open_clip.tokenize(text)
            z = self.encode_with_transformer(tokens.to(self.device))
        else:
            n_prompt = text_prompt.size(0)
            n_img_prompt = image_prompt.size(1)
            prompt_prefix = " ".join(["X"] * n_prompt)
            prompt_prefix_img = " ".join(["*"] * n_img_prompt)
            prompts = [prompt_prefix+ " " +prompt_prefix_img+" "+ text_ for text_ in text]
            tokens = open_clip.tokenize(prompts)
            z = self.encode_with_transformer(tokens.to(self.device),text_prompt.to(self.device),image_prompt.to(self.device))
        return z

    def encode_with_transformer(self, text,text_prompt=None,image_prompt=None):
        if text_prompt == None:
            ...
            n_prompt = text_prompt.size(0)
            n_img_prompt = image_prompt.size(1)
            x[:,1:1+n_prompt,:] = text_prompt
            x[:,1+n_prompt:1+n_prompt+n_img_prompt,:] = image_prompt
    ...
    def encode(self, text,text_prompt=None):
        return self(text,text_prompt)
...
if __name__ == "__main__":
    ...
    prompts = 'without snow drift'
    # tokenized_prompts = torch.cat([open_clip.tokenize(p) for p in prompts])
    tokenized_prompts = open_clip.tokenize(prompts).cuda() # (b,77)
    print(tokenized_prompts)
    with torch.no_grad():
        embedding = model.model.token_embedding(tokenized_prompts) # (b,77,1024)
